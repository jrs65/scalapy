...
# Define complex 'Z' type
ctypedef np.cmplex128 complex16_t
...
def pzhegvx(A, B, type=1, eigvals_only=False, destroy = True, upper = True):
    r"""Compute the eigen-decomposition of a symmetric matrix.
    ...
    cdef DistributedMatrix Al, Bl, evecs
    ...
    cdef double orfac, abstol

    cdef int m, nz

    ## Check input
    assert_type([A, B], np.complex128)
    assert_square([A, B])
    
    Al = A if destroy else A.copy()
    Al = B if destroy else B.copy()

    evecs = DistributedMatrix.empty_like(A)
    evals = np.empty(A.Nr, dtype=np.float64)

    uplo = "U" if upper else "L"
    jobz = "N" if eigvals_only else "V"
    ...
    pdsyevd_(<int>type, jobz, rng, uplo, &(A.Nr),
             <complex16_t *>A._data(), &_ONE, &_ONE, A._getdesc(),
             <complex16_t *>B._data(), &_ONE, &_ONE, B._getdesc(),
             &vl, &vu, &il, &iu,
             &abstol,
             &m, &nz, <double *>np_data(evals),
             &orfac,
             <double *>evecs._data(), &_ONE, &_ONE, evecs._getdesc(),
             &wl, &lwork, &wrl, &lrwork, &wil, &liwork,
             ifail, iclustr, gap, &info);
    
    ## Initialise workspace to correct length
    lwork = <int>wl
    work = <complex16_t *>malloc(sizeof(complex16_t) * lwork)

    lrwork = <int>wrl
    rwork = <int *>malloc(sizeof(double) * lrwork)
