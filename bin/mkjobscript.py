...
nodes = pside**2
ppn = 8
name = "IBU2_%in_%ib_%ip_%it" % (gsize, bsize, pside, nthread)
nomp = nthread
numproc = pside**2
pernode = 1
...
#mpirun --mca btl self,sm,openib -np %(numproc)i -hostfile $PBS_NODEFILE -npernode %(pernode)i python benchmark.py %(gsize)i %(bsize)i %(pside)i %(pside)i %(nomp)i /scratch/jrs65/scalapack/jobtime_%(name)s.dat &> /scratch/jrs65/scalapack/jobout_%(name)s.log
